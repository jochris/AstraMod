...
import { Sparkles, Gamepad2, Smartphone, ShieldCheck, Flame, Filter, Zap } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
        ...
        {/* Featured Section */}
        {!search && !category && activeType === 'all' && featuredApps.length > 0 && (
        ...
        {/* Main Content Area: Category Sidebar / Filter + Grid */}
        <section>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-6 scrollbar-none text-xs">
            ...
            {categories.map((cat) => (
              <Link
                key={cat.name}
                href={`/?category=${encodeURIComponent(cat.name)}${type ? `&type=${type}` : ''}`}
